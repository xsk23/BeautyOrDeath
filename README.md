...
### 1. 通用操作 (Common Controls)

| 按键 | 功能描述 |
| :--- | :--- |
| **W / A / S / D** | 角色移动 |
| **Mouse** | 视距/准星旋转 |
| **Shift (按住)** | **疾跑**：增加移动速度（女巫变身为动物后继承该动物的奔跑速度） |
| **Space (空格)** | **跳跃** / **挣脱**：被猎人陷阱抓住时连按可加快逃脱 / **下车**：作为乘客时跳离宿主 |
| **T** | 切换 **第一人称 (FPS)** / **第三人称 (TPS)** 视角 |
| **Tab (按住)** | 查看对局状态栏（玩家列表、阵营、实时延迟 Ping 及所选技能图标） |
| **Escape** | 打开/关闭 游戏暂停菜单 / 退出输入状态 |
| **/** | 开启局内聊天输入框 |
| **Enter** | 发送聊天消息 |

### 2. 女巫阵营 (Witch Specific)

| 按键 | 功能描述 |
| :--- | :--- |
| **LMB (左键)** | **变身/还原**：短按变身为准星指向的物体；长按（1.5秒）恢复人类形态 |
| **RMB (右键)** | **附身古树**：长按（1秒）附身大型古树成为“驾驶员”。若树已被附身，则加入成为乘客 |
| **Q / E** | 释放战前选择的 **职业技能**（如：迷雾、分身、诅咒穿墙、混乱等） |
| **F** | 激活战前选择的 **特殊道具**（如：隐身斗篷、生命护符、魔法扫帚二段跳等） |
| **V** | **嘲讽 (Taunt)**：发出笑声并获得瞬时回蓝、加速、技能减CD，但会向猎人暴露轮廓 |

### 3. 猎人阵营 (Hunter Specific)

| 按键 | 功能描述 |
| :--- | :--- |
| **LMB (左键)** | **攻击**：使用当前选中的武器（猎枪/蜂蜜枪/拳头）进行开火或挥拳 |
| **RMB (右键)** | **补充弹药**：当手持“蜂蜜枪”且对准蜂蜜罐时，长按可回填弹药 |
| **1 / 2 / 3 或 滚轮** | **武器切换**：在 [1]猎枪、[2]蜂蜜枪、[3]拳头 之间切换 |
| **Q / E** | 释放战前选择的 **职业技能**（如：放置陷阱、召唤猎犬、范围扫描、震荡波等） |
| **F** | **处决**：当靠近被陷阱禁锢的女巫时，按下可执行高伤害处决 |

### 4. 特殊模式 (Special Modes)

| 按键 | 功能描述 |
| :--- | :--- |
| **Tab (聊天时)** | 切换消息频道：**[ALL] 所有人** / **[TEAM] 仅队友** |
| **U** | **观察者模式**：仅管理员/调试状态可用，开启后可自由飞行穿越地图 |
| **Space / Ctrl** | **观察者模式下**：控制上升与下降 |

---

### 💡 核心机制小提示：
*   **挣脱机制**：被陷阱抓住后，你的屏幕会出现进度条，必须快速连按 **Space** 达到要求次数（默认2次，视服务器设置而定）才能恢复行动。
*   **多人协作**：多名女巫可以同时附身于同一棵古树或物体。宿主的移动速度会受到所有乘客 **WASD** 输入的合力加成。
*   **侦察奖励**：女巫通过准星检视地图上的树木可积累进度，每完成一定数量（默认20棵）即可通过弹出 UI 选择一项 **永久属性强化** 或 **技能增益**。
